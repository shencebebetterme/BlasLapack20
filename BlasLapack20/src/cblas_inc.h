...
/*** uint16_t and INT8 extensions ***/
/* convert float array to uint16_t array by rounding */
void   cblas_sbstobf16(const int n, const float  *in, const int incin, uint16_t *out, const int incout);
/* convert double array to uint16_t array by rounding */
void   cblas_sbdtobf16(const int n, const double *in, const int incin, uint16_t *out, const int incout);
/* convert uint16_t array to float array */
void   cblas_sbf16tos(const int n, const uint16_t *in, const int incin, float  *out, const int incout);
/* convert uint16_t array to double array */
void   cblas_dbf16tod(const int n, const uint16_t *in, const int incin, double *out, const int incout);
/* dot production of uint16_t input arrays, and output as float */
float  cblas_sbdot(const int n, const uint16_t *x, const int incx, const uint16_t *y, const int incy);
void   cblas_sbgemv(const enum CBLAS_ORDER order,  const enum CBLAS_TRANSPOSE trans,  const int m, const int n, const float alpha, const uint16_t *a, const int lda, const uint16_t *x, const int incx, const float beta, float *y, const int incy);
...
void   cblas_sbgemm(const enum CBLAS_ORDER Order, const enum CBLAS_TRANSPOSE TransA, const enum CBLAS_TRANSPOSE TransB, const int M, const int N, const int K,
		    const float alpha, const uint16_t *A, const int lda, const uint16_t *B, const int ldb, const float beta, float *C, const int ldc);
}
